...
def get_property_from_db(zpid):
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        property = cursor.execute('SELECT zillow_ID, streetAddress, price, num_beds, num_baths, zestimate, sqft, price_per_sqft, house_type, property_tax, nearby_schools, nearby_cities, images, description, images FROM propertyDetails WHERE zillow_ID = ?', (zpid,)).fetchone()
...
def get_prop_search_history():
    properties = sql_data_to_list_of_dicts("zillow_listings.db", "SELECT * FROM propertyDetails LIMIT 5")
    return properties
